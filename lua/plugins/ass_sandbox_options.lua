...
if (CLIENT) then
	...
	function PLUGIN.OnOffMenu(MENU, CMD, ON_VAL, OFF_VAL, ON_TXT, OFF_TXT)
		...
		if (Items[ SandboxOptions[CMD] ]) then
			Items[ SandboxOptions[CMD] ]:SetImage("gui/silkicons/check_on_s")
		end
	...
	function PLUGIN.AddGamemodeMenu(DMENU)
		...
		DMENU:AddSubMenu( "Give weapons", 		nil, function(NEWMENU) PLUGIN.OnOffMenu(NEWMENU, "sbox_weapons", 	1, 0 ) end )
		DMENU:AddSubMenu( "Allow Player damage",	nil, function(NEWMENU) PLUGIN.OnOffMenu(NEWMENU, "sbox_godmode", 	0, 1 ) end )
		DMENU:AddSubMenu( "Allow PvP Damage",		nil, function(NEWMENU) PLUGIN.OnOffMenu(NEWMENU, "sbox_plpldamage", 	0, 1 ) end )
		DMENU:AddSubMenu( "Allow Noclip",		nil, function(NEWMENU) PLUGIN.OnOffMenu(NEWMENU, "sbox_noclip", 	1, 0 ) end )
		DMENU:AddSubMenu( "Admin has no limits",	nil, function(NEWMENU) PLUGIN.OnOffMenu(NEWMENU, "sbox_admin_nolimits",	1, 0 ) end )
