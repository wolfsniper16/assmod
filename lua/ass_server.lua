...
function CONSOLE:IsOperator()			if (!self:IsValid()) then return true else return false end end
function CONSOLE:IsDonatorGold()		if (!self:IsValid()) then return true else return false end end
function CONSOLE:IsDonatorSilver()		if (!self:IsValid()) then return true else return false end end
...
function ASS_PlayerInitialSpawn( PLAYER )
	...
	if (#player.GetAll() <= 2 && !PLAYER:IsOperator() && util.tobool(ASS_Config["demomode"]) ) then
...
function ASS_PlayerSpeech( PLAYER, TEXT, TEAMSPEAK )
	...
	if (PLAYER:IsOperator() && string.sub(TEXT, 1, prefixlen) == prefix) then
